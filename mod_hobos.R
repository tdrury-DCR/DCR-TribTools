...
HOBO <- function(input, output, session, hobo_path, updir, hobo_db, df_trib_monitoring, baro_table, hobo_table, mayfly_data_dir, mayfly_data_processed, ImportFlagTable, username, userlocation) { # Same as rating info - all in Hydro DB
  ...
  observeEvent(input$import, {
    out <- tryCatch(
      switch(file_type(),
        "baro" = {
          IMPORT_BARO(df_baro = df(), baro_file = input$file, userlocation = userlocation)
        },
        "hobo" = {
          IMPORT_HOBO(df_hobo = df(), df_flags = df_flags(), hobo_txt_file = input$file, userlocation = userlocation)
        },
        "mayfly" = {
          IMPORT_MAYFLY(df_mayfly = df(), mayfly_file = input$file, userlocation = userlocation)
        }
      ),
      error = function(e) e
    )
  ...
  ### Text Outputs ####
  output$file_selection <- renderText({
    file_selected()
  })
