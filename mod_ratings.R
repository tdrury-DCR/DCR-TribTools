...
RATINGS_UI <- function(id) {
  ...
  tagList(
    ...
    div(
      id = "ratings", ### Set div() for Refresh Button ####
...
RATINGS <- function(input, output, session, df_ratings, df_discharges) {
  ...
  output$intro <- renderText({
    paste0("This module utilizes the nls package to determine the nonlinear (weighted) least-squares estimates of the parameters of a nonlinear model.  Once a location is selected, choose which rating to create. An existing rating can be recalculated or a new rating can be created.\n
  ...
  output$pzf_current <- renderText({
    ...
    if (!is.na(current_offset)) {
      ...
      paste0("There is not current rating yet, but the suggested offset is the minimum stage: ", min_stage, "ft")
    }
  })

  output$discharges <- renderDataTable({
    datatable(df_discharges[, c(1:ncol(df_discharges) - 1)]) %>%
      formatDate(columns = c("DateTimeStartET", "DateTimeEndET"), method = "toLocaleString")
  })

  output$ratings <- renderDataTable({
    datatable(df_ratings[, c(1:ncol(df_ratings) - 1)]) %>%
      formatDate(columns = c("DateTimeStartET", "DateTimeEndET"), method = "toLocaleString")
  })
  ...
  datatext <- reactive({
    if (parts() == 1) {
      ...
      l <- list(
        `RATING EQUATION (Part 1):` = data()$eq1,
        `COEFFICIENT STATS:` = data()$t_sum1,
        `COEFFICIENT 95% CONFIDENCE INTERVALS:` = data()$conf_int1,
