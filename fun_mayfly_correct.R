...
data_correct_summary <- function(parameter) {
  ...
  # Only filter > 2024-01-01 for Wachusett
  ...
  if (userlocation == "Wachusett") {
    if (parameter == "Stage_ft") {
      df <- db_mayfly %>%
        ...
        group_by(Location) %>%
        summarize(
          "MinDateTimeUTC" = min(DateTimeUTC, na.rm = TRUE),
    ...
    if (parameter == "Stage_ft") {
      df <- db_mayfly %>%
        filter(is.na(Stage_ft), is.na(Discharge_cfs)) %>%
        group_by(Location) %>%
        summarize(
          "MinDateTimeUTC" = min(DateTimeUTC, na.rm = TRUE),
          "MaxDateTimeUTC" = max(DateTimeUTC, na.rm = TRUE)
      ...
      df <- db_mayfly %>%
        filter(is.na(Conductivity_uScm), !is.na(RawConductivity_uScm)) %>%
        group_by(Location) %>%
        summarize(
          "MinDateTimeUTC" = min(DateTimeUTC, na.rm = TRUE),
          "MaxDateTimeUTC" = max(DateTimeUTC, na.rm = TRUE)
...
preview_plot <- function(loc, par, sum_loc, df_fp, df_trib_monitoring) {
  ...
  if (userlocation == "Wachusett") {
    df_hobo <- hobo_tbl %>%
      filter(
        between(DateTimeUTC, min_dt, max_dt),
        Location == loc
      ) %>%
      select(2, 3, 6, 7) %>%
      collect()
