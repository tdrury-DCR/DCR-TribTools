...
### NOTES
### Quabbin DB changes;
  # Drop datasource col from tblHOBO and tblHOBO_BARO 
  # Update DateTime to DateTimeUTC
  # 
...
RATINGS <- function(input, output, session, df_ratings, df_discharges){
  ...
  site_choices <- reactive({
    c(df_discharges %>% .$Location) %>% unique() %>% sort() %>% paste()
    })
  ...
  output$pzf_current <- renderText({
    ...
    current_offset <- df_ratings$a1[df_ratings$MWRA_Loc == input$site & df_ratings$Current == TRUE]
    min_stage <- df_ratings$MinStage[df_ratings$MWRA_Loc == input$site & df_ratings$Current == TRUE]
